...
import { dummyIntervals } from './data/dummyIntervals';
import {
	convertIntervalsToEvents,
	initializeIntervalTree,
	queryIntervalsByTimeRange
} from './utils/convertIntervalsToEvents';
...
const App: React.FC = () => {
	...
	// Initialize the interval tree on component mount
	...
	const handleFetchEvents = useCallback(async () => {
		...
		try {
			...
			// Use the interval tree for fast O(log n + k) queries
			...
			const fetchedEvents = convertIntervalsToEvents(filteredIntervals);
			setEvents(fetchedEvents);
			setQueryTimeMs(queryDuration);
		} catch (err) {
		...
		if (events.length > 0) {
			return (
				<>
					{queryTimeMs !== null && (
						<div className="mb-4">
							<QueryStats queryTimeMs={queryTimeMs} resultCount={events.length} />
						</div>
					)}
		...
		<div className="min-h-screen bg-gray-900 text-gray-100 p-4 sm:p-6 lg:p-8">
			<div className="max-w-4xl mx-auto">
				<header className="text-center mb-8">
					...
					<p className="mt-2 text-gray-400">Find events within a specific time frame.</p>
				</header>
